...
test("callback ingress reconciles run artifact from running to completed when callback is accepted", async () => {
    ...
    JSON.stringify(
      {
        ...
        state: "running",
        lastEvent: null,
        lastSummary: "Attach-run dispatched; terminal callback owned by OpenCode-side plugin",
        ...
        envelope: {
          ...
          callback_target_session_key: "agent:fullstack:telegram:direct:5165741309",
  ...
  try {
    ...
    assert.match(String(persisted.lastSummary || ""), /Terminal callback materialized/);
    assert.equal(persisted.callbackOk, true);
    assert.equal(persisted.callbackStatus, 200);
    assert.match(String(persisted.callbackBody || ""), /"runId":"vaa24-image-catalog-20260324t2031"/);
  } finally {
