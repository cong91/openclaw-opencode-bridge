...
test("OpenCode-side plugin mirrors callback outcome into OpenClaw-side audit file", async () => {
	...
	globalThis.fetch = async () =>
		({
			ok: true,
			status: 200,
			text: async () => JSON.stringify({ ok: true, runId: "hook-run-1" }),
		}) as any;
	...
	try {
			...
			event: {
				...
				properties: {
					info: {
						id: "sess-audit",
						title:
							"demo runId=run-audit taskId=task-audit requested=fullstack resolved=fullstack callbackSession=agent:fullstack:opencode:fullstack:callback:session-audit callbackSessionId=session-audit callbackRelaySession=agent:fullstack:telegram:direct:5165741309 callbackRelaySessionId=session-audit projectId=proj-audit repoRoot=/tmp/repo-audit workflowId=wf-audit stepId=step-audit",
					},
				},
			},
		});
		...
		const callbackRows = rows.filter(
			(row) => row.callbackOk === true || row.callbackStatus !== undefined,
		);
		assert.equal(callbackRows.length >= 1, true);
		const target =
			callbackRows.find((row) => row.runId === "run-audit") || callbackRows[0];
		...
		if (oldOpenclawAudit === undefined)
			delete process.env.OPENCLAW_BRIDGE_OPENCLAW_AUDIT_PATH;
		else process.env.OPENCLAW_BRIDGE_OPENCLAW_AUDIT_PATH = oldOpenclawAudit;
