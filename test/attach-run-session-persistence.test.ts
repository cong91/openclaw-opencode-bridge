...
test("startExecutionRun persists resolved opencode session id for attach-run", async () => {
      ...
      res.end(JSON.stringify([
        {
          ...
          title: `task-attach-session runId=run-attach-session-1 taskId=task-attach-session-1 requested=creator resolved=build callbackSession=agent:creator:telegram:direct:5165741309 callbackSessionId=04577833-156b-4989-80ab-13067a0949f3 callbackDeliver=false projectId=proj-attach-session repoRoot=${repoRoot}`,
          directory: repoRoot,
  ...
  try {
    ...
    const runPath = join(stateDir, "opencode-bridge", "runs", "run-attach-session-1.json");
