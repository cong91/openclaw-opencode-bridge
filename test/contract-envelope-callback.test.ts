...
test("opencode_build_envelope contract preserves origin/requested/resolved and callback target routing", async () => {
  ...
  try {
    ...
    const callback = buildHooksAgentCallback({ event: "task.progress", envelope });
    assert.equal(callback.agentId, "creator");
    assert.equal(callback.sessionKey, "session:origin:key");
    assert.equal(callback.sessionId, "session-origin-id");
