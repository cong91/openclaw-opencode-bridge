...
function setupHarness(configFile: any) {
  const tempRoot = mkdtempSync(join(tmpdir(), "opb-backward-compat-"));
...
test("opencode_run_status handles legacy artifacts with opencodeSessionId but no sessionId", async () => {
  const mock = await withMockServer((req, res) => {
    ...
    if (url === "/session") { res.setHeader("content-type", "application/json"); res.end(JSON.stringify([{ id: "sess-legacy-1", title: "legacy run", time: { updated: Date.now() } }])); return; }
  ...
  try {
    ...
    writeFileSync(join(runDir, "run-legacy-1.json"), JSON.stringify({
      taskId: "task-legacy-1",
      runId: "run-legacy-1",
      state: "running",
      updatedAt: new Date().toISOString(),
      opencodeSessionId: "sess-legacy-1",
      envelope: { task_id: "task-legacy-1", run_id: "run-legacy-1", agent_id: "build", requested_agent_id: "creator", resolved_agent_id: "build", session_key: "hook:opencode:build:task-legacy-1", origin_session_key: "session:origin:legacy", callback_target_session_key: "session:origin:legacy", project_id: "proj-legacy", repo_root: "/tmp/legacy", opencode_server_url: mock.baseUrl }
    }, null, 2), "utf8");
    const tool = harness.tools.get("opencode_run_status");
    assert.ok(tool);
    const payload = parseToolJson(await tool!.execute("test", { runId: "run-legacy-1", opencodeServerUrl: mock.baseUrl }));
    assert.equal(payload.sessionId, "sess-legacy-1");
    assert.ok(payload.warnings.includes("legacy_artifact_sessionid_backfilled_from_opencodesessionid"));
  } finally {
    await mock.close();
    harness.cleanup();
  }
