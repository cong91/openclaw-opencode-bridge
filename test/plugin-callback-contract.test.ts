...
test("OpenCode-side plugin correlates session tags and callbacks once on session.idle", async () => {
  ...
  try {
    ...
    await plugin.event({
      event: {
        ...
        properties: {
          info: {
            id: "sess-1",
            title: "demo runId=run-1 taskId=task-1 requested=fullstack resolved=fullstack callbackSession=agent:fullstack:telegram:direct:5165741309 callbackSessionId=session-123",
          },
        },
      },
    });
    ...
    assert.equal(callbackHits[0].sessionKey, "agent:fullstack:telegram:direct:5165741309");
    assert.equal(callbackHits[0].sessionId, "session-123");
    assert.ok(rows.some((r) => r.phase === "event_seen" && r.event_type === "session.created"));
